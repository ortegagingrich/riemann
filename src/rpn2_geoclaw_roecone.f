...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      !local for simple fwave solver:
      double precision hsqrtL, hsqrtR, vhat
...
c        
        ...
        if ((hL<roe_min_depth) .or. (hR<roe_min_depth)) then
            go to 13 !use full solver if either state is too shallow
        endif

        !are the momenta too great relative to depth?
        !(risk of transonic rarefaction)
        if((abs(huL)>roe_mom_rat*(hL-roe_min_depth)).or.
     &     (abs(huR)>roe_mom_rat*(hR-roe_min_depth))) then
            go to 13 !use full solver if momentum is too great
        endif

        !are the depths sufficiently close that the roe middle state
        !remains a good approximation of the intersection of the
        !nonlinear loci/integral curves?
        if(abs(hR-hL).ge.(roe_depth_frac*min(hR, hL))) then
            go to 13 !use the full solver
        endif
        
        !at this point, it should be safe to try the roe solver
...
c      	         bL=hstartest+bR
       	  ...
       	  uhat=(hsqrtL*uL + hsqrtR*uR)/(hsqrtL+hsqrtR) ! Roe average
       	  vhat=(hsqrtL*vL + hsqrtR*vR)/(hsqrtL+hsqrtR) ! Roe average
         ...
         ! use augmented solver only if depth is shallow on one side:
...
!515         format("++sw",4e25.16)
         ...
         go to 30 
c      else
 14      continue !continue for Roe solver
...
!$OMP END CRITICAL (roesolvescounter)
          ...
          del1 = delhu
          del2 = delphi + g*0.5d0*(hR+hL)*delb
          del3 = hR*uR*vR - hL*uL*vL
          ...
          !decompose flux difference + source into Roe eigenvectors:
          ! should we use sE1 and sE2 in place of sRoe1,2 = uhat \pm chat?
    
          beta1 = ((uhat+chat)*del1 - del2) / (2.d0*chat)
          !at this point, we can check to see if the middle state is
          !inside the permissible cone for the roe solver
...
!$OMP END CRITICAL (retrysolvescounter)
          ...
          beta3 = (-(uhat-chat)*del1 + del2) / (2.d0*chat)
    
          fwave(1,1,i) = beta1
          fwave(mu,1,i) = beta1 * (uhat - chat)
          ...
          fwave(1,3,i) = beta3
          fwave(mu,3,i) = beta3 * (uhat + chat)
          fwave(nv,3,i) = beta3 * vhat
          s(3,i) = uhat + chat
    
c        endif

 30    continue  ! jump to here if dry on both sides
       enddo  ! end loop on i
